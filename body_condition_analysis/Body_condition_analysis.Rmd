...
indv_df <- readRDS(file.path("..","calculate_body_condition", "outputs", "collated_allMC.rds"))
...
df.summary_meta <- df.summary %>% left_join(indv_meta, by = "AID") %>% 
  ...
  Focal_Length, Iw, Sw, date_source, Reproductive_Class, TL.mean, TL.lower, TL.upper, SW.mean, SW.lower, SW.upper, BAIpar.HT.mean,
...
exSWstd <- ggplot(ex_output, aes(SW)) + # note that "SW" here was calculated in CollatriX and is already standardized 
  stat_halfeye(fill = "steelblue", alpha = 0.75) + 
